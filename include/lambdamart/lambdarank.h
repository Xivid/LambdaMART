...
class LambdaRank {
    ...
    void get_derivatives(float* currentScores, float* gradients, float* hessians);
    void get_derivatives_one_query(float* scores, float* gradients,
                                    float* hessians, sample_t query_id);
    std::vector<float> eval(float* scores);
    ...
    private:
        ...
        //void Init(const std::vector<float>& input_label_gain);

        // Calculates the DCG score at position k, given the rank label and score
        float cal_dcg_k(int k, const label_t* label, const float* score, sample_t num_data);
        ...
        void cal_dcg(const std::vector<int>& ks, const label_t* label, const float* score,
