...
class LambdaRank {
    ...
    void get_derivatives(score_t* currentScores, gradient_t* gradients, gradient_t* hessians);
    void get_derivatives_one_query(score_t* scores, gradient_t* gradients,
                                    gradient_t* hessians, sample_t query_id);
    std::vector<score_t> eval(score_t* scores);

    private:
        const sample_t* boundaries_;
        sample_t  num_queries_;
        label_t* label_;
        std::vector<float> inverse_max_dcg_;
        std::vector<std::vector<float>> eval_inverse_max_dcg_;
        // NDCG related fields
        std::vector<float> label_gain_;
        ...
        float cal_dcg_k(int k, const label_t* label, const score_t* score, sample_t num_data);
